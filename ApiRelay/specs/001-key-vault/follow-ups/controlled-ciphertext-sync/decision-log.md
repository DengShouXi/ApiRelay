...
**Status**: `v1.13.11` 插入顺序 D00A 已获批准；同步 G1 D01–D17 仍待产品负责人逐项裁决，本文件不是同步实施授权。
...
| 同步前是否先稳定结构 | 历史 `v1.13.9` 存在 4,595 行 View、1,893 行 privacy controller，且没有 XCUITest target；`v1.13.10@8e6c221` 已完成结构稳定化与真正的 UI 自动测试 | 已采用独立 `v1.13.10`，其真机系统边界仍属发布闸门；随后插入 `v1.13.11` 购买权益，不能直接启动同步 | 前序已执行；同步 G1 仍未裁决 |
...
| --- | --- | --- | --- | --- | --- |
| D00 | 是否启动独立 `v1.13.10` 结构稳定化 | 从 `v1.13.9` 关账 SHA 建干净分支，只做拆分、状态单一来源与真实 XCUITest；不改产品行为 | **已批准并启动** | 不在原脏工作树续做；不把同步、认证语义或 UI 改版混入 | 产品负责人 / 2026-09-24 / 明确要求“实现下一阶段的代码，完成后测试，进一步修改直至成功” |
| D00A | 是否在同步前插入独立购买权益验收 | `v1.13.10@8e6c221` 关账后建立干净 `v1.13.11`，只验收购买权益；其关账和远端复核通过后才可作为未来同步实施输入 | **已批准：插入 `v1.13.11`** | 不把购买草稿并入 `13.10`，不从 `13.10` 直接启动同步，不把购买与同步合线 | 产品负责人 / 2026-09-24 / 审核“13.11 只做购买权益 → 1.14 先定方案再做同步 → UI 最后”的计划后要求按计划实施、检测和上传；回写 `release-sequence.md`、本表及受影响同步入口，`ZL01/14` 同步更新 |
| D01 | 同步版本归属与后续顺序 | 已批准的前序为 v1.13.9 → v1.13.10 → v1.13.11；候选为随后把 Stage 1 v1.14 用于受控同步，产品 V2 仍为 2.0 用量看板，UI 最后单列 | 待定（不重复裁决 D00A） | 待定 | 待定 |
...
本次只回写已批准的 D00A 版本插入及其受影响的草案路牌；D01–D17 的安全/产品权威回写，仍须全部裁决后另行授权。
