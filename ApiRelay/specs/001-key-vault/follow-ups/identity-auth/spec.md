...
**Git**：2026-09-10 在 **`v1.13.8`** 从已关账的 `v1.13.7` 开线并定稿。**落地（W0 与代码）计划在尚未创建的 `v1.13.9`**，不是现在就在 13.8 实现，也不是 `v1.14` / `P14`。  
锁加固 Wave 1–3 已关在 `v1.13.7`，MUST NOT 写回那条。

**Created**: 2026-09-10

**Status**: Active — 定稿入口 [`../../../playbooks/身份验证方案.md`](../../../playbooks/身份验证方案.md)。宪法 / `001-key-vault/spec.md` 的 **FR 编号正文尚未改**（W0，计划 `v1.13.9`）。实现时以本目录与 playbook 为准，不要按已覆盖的旧句写新代码。不要把 `v1.13.9` 写成已经创建。
...
| 旧句（仍写在宪法 / FR 正文里） | 2026-09-10 定稿（当时在 v1.13.8 拍板；落地计划 v1.13.9） |
|---|---|
