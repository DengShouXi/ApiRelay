...
在实现阶段定稿；`CL-004`（应用专属主密码）本期明确不做。
> **已被 13.9 覆盖：** 上句「本期明确不做」是 2026-08-04 当时验收口径，不得再当现行。应用密码已做（DC-007、FR-036～038）。现行见 [`follow-ups/identity-auth/spec.md`](../follow-ups/identity-auth/spec.md)。
影响架构的 `CL-001` / `CL-002` 已由产品
...
> **已被 13.9 覆盖：** 现行是三行设置 + 四档（出厂设备验证，无「仅生物识别」）+ 独立取用验证。当时「三档」与「单一设置项管查看+复制」不得再当现行。
> **已被 U1 覆盖（2026-09-15）：** 列表定名「生物验证或设备密码」；密码档立即设密。
> **已被 U2 / U11 覆盖（2026-09-15 / 2026-09-22）：** 不再始终可见应用主密码行；界面称「应用密码」；点两种档进同一页；组合档系统路径只启动一次 `deviceOwnerAuthentication`。现行见宪法 v2.17.0、FR-003 / FR-021、[`follow-ups/identity-auth/spec.md`](../follow-ups/identity-auth/spec.md)。
...
| **应用主密码**（DC-007） | 门闩由三档变四档。<br>**已被 13.9 覆盖：** 当时四档含「仅生物识别」；现行四档无该档。现行见 [`follow-ups/identity-auth/spec.md`](../follow-ups/identity-auth/spec.md)。 | FR-036～038、research §1.2 |
