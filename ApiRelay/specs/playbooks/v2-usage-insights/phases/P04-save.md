...
| **小阶段分支（第 3 层，本次上传目标）** | `v0.2.4` |
| 远程 | `origin` |
| 台账 | `ApiRelay/specs/playbooks/BRANCHES.md` |
| 对应实现提示词 | `phases/P04-密钥探活.md`（本对话不实现） |
| 本阶段测试目录 | `ApiRelay/ApiRelayTests/V2/Phase04_Health/` |

## A. 更新分支备注（必须英 → 中）

编辑 `BRANCHES.md`，为小阶段分支 **`v0.2.4`** 写入/更新：

1. **English**（2–4 句）：相对上一小阶段多了什么、不含什么、何时 checkout `v0.2.4`。  
2. **简体中文**：同样信息。  
...
2. 若当前不在 `v0.2.4`：从大阶段线创建或检出  
   `git checkout v0.2` && `git pull` && `git checkout -B v0.2.4`  
   （`-B` 把本小阶段分支指到当前大阶段最新提交；**经我确认后再执行**）。  
3. 暂存：本 Phase 实现 + 对应 `ApiRelayTests/V2/Phase04_*/` 测试 + `BRANCHES.md`。  
4. **禁止**加入：`ApiRelay/build/`、`ApiRelay/DebugScratch/`、`__pycache__/`、`spec-kit-0.15.2/`、密钥、`.env`。  
5. 展示 `git diff --cached --stat`，**等我同意后再 commit**。  
6. 建议 message：

```text
feat(v0.2.4): Phase 4 密钥探活 checkpoint
```
...
8. （推荐）同步大阶段分支尖端：`git checkout v0.2` && `git merge v0.2.4` && `git push origin v0.2`，再回到 `v0.2.4`。

## C. 完成汇报

- 小阶段分支 `v0.2.4` @ `<hash>`（远程已有）  
- `BRANCHES.md` 是否已含英+中  
- 测试是否只在 `ApiRelayTests/V2/Phase04_*/`  
- 下一步：下一 Phase 的**实现**提示词；新小阶段将上传到 `v0.2.5`  
