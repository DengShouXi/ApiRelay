...
|----|----------------|
| 工作分支（第 2 层） | `v0.2` |
| 检查点 tag（第 3 层） | `v0.2.8` |
| 远程 | `origin` |
| 台账 | `ApiRelay/specs/playbooks/BRANCHES.md` |
| 对应实现提示词 | `phases/P08-上架与收尾.md`（仅作对照，本对话不实现） |

## A. 更新分支备注（必须英 → 中）

编辑 `BRANCHES.md`：在 checkpoint tags 表（或「Current refs」下）为 **`v0.2.8`** 写入/更新一条：

1. **English**（2–4 句）：相对上一检查点多了什么、刻意不含什么、何时该 `git checkout v0.2.8` 回看。  
2. **简体中文**：同样信息，口语清楚。  
3. 表格字段：`Tag` = `v0.2.8`，`Commit` = 本步 commit 后的 hash（先占位，commit 后补全），`Phase` = Phase 8 上架与收尾。
...
```text
feat(v0.2): Phase 8 上架与收尾 checkpoint

Freeze Phase 8 on stage branch v0.2; ledger updated for tag v0.2.8.
```

## C. 打 tag 并上传

1. Annotated tag（第三层是 **tag**，禁止新建同名分支 `v0.2.8`）：

```bash
git tag -a v0.2.8 -m "Phase 8 checkpoint: 上架与收尾"
```
...
3. 用 `git ls-remote --heads origin v0.2` 与 `git ls-remote --tags origin v0.2.8` 核对远程已有；把 **commit hash** 写回 `BRANCHES.md` 后若有改动再补一次小 commit + `git push origin v0.2`（**不要**移动已推送的 tag，除非我明确要求）。
...
- 新建分支 `v0.2.8` / `v0.2` / `v0.3` / `v0.4`（未轮到的大阶段不要建）  
