...
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-09-05：**Fake→`DebugSupport/Fakes`**；`重大历史问题`→`事故复盘`；真机抽查修自动锁计时与多任务遮罩挡解锁）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：真机再验锁；备案与商店 1.0.1 不急；旁账剩余 `tools/` 是否入库（本机已够用可继续不入库）。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
