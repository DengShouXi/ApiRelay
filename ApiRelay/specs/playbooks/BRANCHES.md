...
> **工作分支**：`v1.13.8` — 身份验证方案（三行：验证方式 / 自动锁定 / 取用验证；出厂默认设备验证）。规格见 `身份验证方案.md`。商店号仍 **1.0.1**；源码构建号仍 **9**。冻结：`v1.13` … `v1.13.7` 与 tag `release/1.0.0` 不动。  
> **`v1.14`**：已开、停放，尖端与 13.6 相同；留给云同步架构，**不要在这条上改锁或验证**。  
> **下一步**：`v1.13.8` 先 W0 规格（宪法 / FR）再实现。Archive **1.0.1 (9)** 建议等本线验证改完。卫生 T008 仍欠。另授权才 ff `main`、打 tag、点 ASC「提交审核」。  
...
| `v1.13.8` | `bca9401` | 2026-09-10 | EN: Hotfix 13.8 opened — identity-auth scheme (three settings rows; factory default device-owner; reveal reuse same detail page; admin credentials stay mandatory device-owner). Specs in `身份验证方案.md`. No Swift yet. Store still **1.0.1**, build still **9**. / 中文：热修 13.8 开线 — 身份验证方案（三行；出厂设备验证；取用同页复用；管理类凭证仍强制设备主人）。规格在 `身份验证方案.md`。尚未改 Swift。商店号仍 1.0.1，构建号仍 9 | 工作分支（开线后 tip 对齐本次提交）；不覆盖 `v1.13.7`；`main` / tag 不动 |
| `v1.14` | `f80b1a6` | 2026-09-10 | EN: Parked product line for later CloudKit architecture. Same tip as 13.6; no unique commits. Lock hardening stays on v1.13.7; identity-auth stays on v1.13.8. / 中文：停放。预留给云同步架构，与 13.6 同 tip、无独有提交。锁加固在 13.7；身份验证在 13.8 | 停放；不要在这条上改锁或验证 |
| `v1` | `bca9401` | 2026-09-10 | EN: Stage1 tip follows v1.13.8 open (same commit as 13.7 build-9 until 13.8 docs land). / 中文：大阶段 1 tip 跟随 13.8 开线（首笔 docs 落地前与 13.7 构建号 9 同 hash） | `main` / `release/1.0.0` 不动 |
