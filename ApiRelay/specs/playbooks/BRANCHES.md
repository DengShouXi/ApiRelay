...
| `v1.13.7` | `f80b1a6` | 2026-09-10 | EN: Hotfix 13.7 Wave 1 — persist-then-apply when weakening synced security settings; session lock rejects consumer-tool / trash / assignment / reorder writes and skips expired purge; snapshot cover split from auto-lock timer; authentication-in-progress is not idle-cancel. Constitution v2.11.0, FR-069–072. 233 tests green. Multi-window covers stay Wave 2. Store version still **1.0.1**, source build still **7**. / 中文：热修 13.7 Wave 1 — 降低已同步安全等级须保存成功才改锁态；使用方/回收站/指派/排序过闸，过期清扫锁下跳过；快照与计时拆开；认证中不当闲置取消。宪法 v2.11.0，FR-069～072。233 个单测绿。多窗口遮罩留 Wave 2。商店号仍 1.0.1，构建号仍 7 | 工作分支（Wave 1 入库后 tip 对齐本次提交）；不覆盖 `v1.13.6`；`main` / tag 不动 |
...
| `v1` | `f80b1a6` | 2026-09-10 | EN: Stage1 tip follows v1.13.7 after Wave 1 upload. / 中文：大阶段 1 tip 跟随 13.7 | `main` / `release/1.0.0` 不动 |
