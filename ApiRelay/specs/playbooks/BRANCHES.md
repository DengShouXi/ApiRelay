...
> **下一步**：现有 TestFlight **1.0.1 (5)** 仍可手测（不含时长 JSON、不含卫生）。发出含时长字段的下一包前，须 CloudKit Production additive Deploy，再 Archive **1.0.1 (6)**（应含卫生）。双设备手测（T008）装含本线的包再做。`replicaSeed` 本热修不做。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」。  
