...
> **工作分支**：`v1.13.8` — **项目治理体系整理**（本地尚未上传）。身份验证方案已于 2026-09-10 在本线定稿，见 `身份验证方案.md`；W0 与 Swift **落地计划在尚未创建的 `v1.13.9`**。商店号仍 **1.0.1**；源码构建号仍 **9**。冻结：`v1.13` … `v1.13.7` 与 tag `release/1.0.0` 不动。  
...
| `v1.13.8` | `7ca8727` | 2026-09-10 | EN: Hotfix 13.8 opened with identity-auth scheme finalized (three settings rows; factory default device-owner; reveal reuse same detail page; admin credentials stay mandatory device-owner). Specs in `身份验证方案.md` and `001-key-vault/follow-ups/identity-auth/`. Current work on this branch is project-governance tidy-up; W0/Swift are planned for not-yet-created `v1.13.9`. No Swift yet. Store still **1.0.1**, build still **9**. / 中文：热修 13.8 开线时定稿身份验证方案（三行；出厂设备验证；取用同页复用；管理类凭证仍强制设备主人）。规格在 `身份验证方案.md` 与 `follow-ups/identity-auth/`。**现行工作是治理整理**；W0 与代码计划在尚未创建的 `v1.13.9`。尚未改 Swift。商店号仍 1.0.1，构建号仍 9 | 工作分支（治理整理，未上传）；不覆盖 `v1.13.7`；`main` / tag 不动 |
...
| `v1` | `7ca8727` | 2026-09-10 | EN: Stage1 tip follows v1.13.8 open. / 中文：大阶段 1 tip 跟随 13.8 开线 | `main` / `release/1.0.0` 不动 |
