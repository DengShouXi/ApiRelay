...
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-09-04 续：**H2 止血已执行**——`AppRuntime` 环境收口、测试碰生产 Keychain/UserDefaults 当场崩、`RevealPolicy.noVerification`、工程级警告当错误）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：备案、商店热修 1.0.1；**H3a～H4 / 99**；旁账（FakeSupport / 目录改名 / `tools/`）。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
