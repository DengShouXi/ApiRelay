...
> 更新时间：2026-09-24。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0** 仍在架上）。
> **工作分支**：`v1.13.10` — 从 `v1.13.9` 关账 SHA `25fe006896a816475e269e155530c1464cc19304` 建立的独立结构稳定化分支；只补 XCUITest、拆职责过载文件并统一状态来源，不改产品功能。`v1.13.9` 密码/身份验证已冻结。商店号仍 **1.0.1**；源码构建号仍 **9**。当前状态只看 [`ZL01/14`](../../../ZL01_具体说明/14-项目当前状态.md)。
> **`v1.14`**：已开、停放，尖端与 13.6 相同；留给云同步架构，**不要在这条上改锁或验证**。
> **当前任务：** 只看 [`ZL01_具体说明/14-项目当前状态.md`](../../../ZL01_具体说明/14-项目当前状态.md)。本页不另写动态下一步。另授权才 ff `main`、打 tag、点 ASC「提交审核」。卫生 T008 仍欠。含新验证的 Archive 终验等 `v1.13.9` 验收并另授权后再做。
> **App Store Connect**：1.0.1 (5) 已由产品负责人上传；「提交审核」须另授权。内购首次挂审若仍卡住，不阻塞本仓库 `release/1.0.0` 冻结。
...
- 上架后热修 git：[`热修-save.md`](./热修-save.md)  
- 听到「上传」时的顺序（对照规格 → 同步说明 → git）：[`ZL01_具体说明/09-上传流程.md`](../../../ZL01_具体说明/09-上传流程.md) §1  
