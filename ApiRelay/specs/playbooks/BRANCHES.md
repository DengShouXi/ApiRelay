...
> 更新时间：2026-09-05。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。  
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0**）。  
> **工作分支**：`v1.13.3`（从 `v1.13.2` tip `9d4bf78` 开出）— 外观逐项 + 冷启动假锁修复；用户逐项口述，一次只改一类。冻结：`v1.13` / `v1.13.1` / `v1.13.2` 与 tag `release/1.0.0` 不动。  
> **下一步**：外观按用户条目推进；备案与商店 1.0.1、`tools/` 入库仍不急。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
