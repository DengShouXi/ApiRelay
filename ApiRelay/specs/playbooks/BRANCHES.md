...
> 更新时间：2026-09-08。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。  
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0** 仍在架上）。  
> **工作分支**：`v1.13.4`（从 `v1.13.3` tip `767338c` 开出）— 工程商店号钉为 **1.0.1**、构建号 **5**；Release Archive 预览修复。冻结：`v1.13` / `v1.13.1` / `v1.13.2` / `v1.13.3` 与 tag `release/1.0.0` 不动。  
> **下一步**：TestFlight 真机抽查 1.0.1 (5)；`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」。  
> **App Store Connect**：1.0.1 (5) 已由产品负责人上传、处理中；「提交审核」须另授权。内购首次挂审若仍卡住，不阻塞本仓库 `release/1.0.0` 冻结。
