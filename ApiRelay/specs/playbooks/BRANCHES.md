...
> 更新时间：2026-08-27。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。  
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0**）。  
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-08-27 快照：`ZL01` 的 `10`/`11` 入库，文档整理未关账）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：继续改 `10`/`11` 与剩余问题；全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
