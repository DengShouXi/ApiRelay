...
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-09-04 续：Mac ⌘N 接到新建；写 `01-事故复盘`；**H1 验证地基已执行**——`selfcheck.sh`、收尾规则 `/closeout`、需求模板「前提与失败预案」、implement Done When）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：备案、商店热修 1.0.1；**H2～H4 / 99**；旁账（FakeSupport / 目录改名 / `tools/`）。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
