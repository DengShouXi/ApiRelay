...
> **工作分支**：`v1.13.4` — 商店号仍 **1.0.1**；已传 TestFlight 的是构建 **5**；源码构建号已钉 **6**。本线已入库时长子页 / 权益放行 / 规格防回退，以及「上传」三步（对照规格 → 同步本夹 → git）。CloudKit 同步卫生**未**进本线。冻结：`v1.13` / `v1.13.1` / `v1.13.2` / `v1.13.3` 与 tag `release/1.0.0` 不动。  
> **下一步**：现有 TestFlight **1.0.1 (5)** 仍可手测（不含时长 JSON）。发出含时长字段的下一包前，须 CloudKit Production additive Deploy，再 Archive **1.0.1 (6)**。同步卫生另开热修，不要盖本线、不要开 `v1.14`。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」。  
