...
> 更新时间：2026-09-10。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。  
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0** 仍在架上）。  
> **工作分支**：`v1.13.8` — **项目治理体系整理**。身份验证方案已于 2026-09-10 在本线定稿，见 `身份验证方案.md`；W0 与 Swift **落地计划在尚未创建的 `v1.13.9`**。商店号仍 **1.0.1**；源码构建号仍 **9**。冻结：`v1.13` … `v1.13.7` 与 tag `release/1.0.0` 不动。当前上传状态只看 [`ZL01/14`](../../../ZL01_具体说明/14-项目当前状态.md)。  
...
| `v1.13.8` | `26be75f` | 2026-09-10 | EN: Hotfix 13.8 opened with identity-auth scheme finalized (three settings rows; factory default device-owner; reveal reuse same detail page; admin credentials stay mandatory device-owner). Specs in `身份验证方案.md` and `001-key-vault/follow-ups/identity-auth/`. Follow-up: project-governance tidy-up (ZL00, ZL02 archive placement, upload A/B loop); W0/Swift remain planned for not-yet-created `v1.13.9`. No Swift. Store still **1.0.1**, build still **9**. / 中文：热修 13.8 开线时定稿身份验证方案（三行；出厂设备验证；取用同页复用；管理类凭证仍强制设备主人）。规格在 `身份验证方案.md` 与 `follow-ups/identity-auth/`。续：项目治理整理（ZL00、事故档案归位、上传闭环）；W0 与代码计划在尚未创建的 `v1.13.9`。尚未改 Swift。商店号仍 1.0.1，构建号仍 9 | 工作分支（治理整理）；内容 tip 为提交 A；不覆盖 `v1.13.7`；`main` / tag 不动 |
...
| `v1` | `26be75f` | 2026-09-10 | EN: Stage1 tip follows v1.13.8 content commit A. / 中文：大阶段 1 tip 跟随 13.8 内容提交 A | `main` / `release/1.0.0` 不动 |
