...
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-09-04 续：**H3a 服务改认接口已执行**——协议补缺口，服务与组装根改 `any XxxServing`）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：备案、商店热修 1.0.1；**H3b～H4 / 99**；旁账（目录改名 / `tools/`）。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
