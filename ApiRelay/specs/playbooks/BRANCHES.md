...
> **工作分支**：`v1.13.7` — 锁加固 Wave 1 已入库（宪法 v2.11.0，FR-069～072）。下一步 Wave 2 每窗遮罩。商店号仍 **1.0.1**；源码构建号仍 **7**。冻结：`v1.13` … `v1.13.6` 与 tag `release/1.0.0` 不动。  
> **`v1.14`**：已开、停放，尖端与 13.6 相同；留给云同步架构，**不要在这条上改锁**。  
> **下一步**：Wave 2–3；锁验收矩阵与卫生 T008 仍欠。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」、改构建号 9。  
...
| `v1.13.7` | `0c2c753` | 2026-09-10 | EN: Hotfix 13.7 Wave 1 — persist-then-apply when weakening synced security settings; session lock rejects consumer-tool / trash / assignment / reorder writes and skips expired purge; snapshot cover split from auto-lock timer; authentication-in-progress is not idle-cancel. Constitution v2.11.0, FR-069–072. 233 tests green. Multi-window covers stay Wave 2. Store version still **1.0.1**, source build still **7**. / 中文：热修 13.7 Wave 1 — 降低已同步安全等级须保存成功才改锁态；使用方/回收站/指派/排序过闸，过期清扫锁下跳过；快照与计时拆开；认证中不当闲置取消。宪法 v2.11.0，FR-069～072。233 个单测绿。多窗口遮罩留 Wave 2。商店号仍 1.0.1，构建号仍 7 | 工作分支（Wave 1 代码 tip）；其后 docs 提交不计入本列；不覆盖 `v1.13.6`；`main` / tag 不动 |
| `v1.14` | `f80b1a6` | 2026-09-10 | EN: Parked product line for later CloudKit architecture. Same tip as 13.6; no unique commits. Lock hardening stays on v1.13.7. / 中文：停放。预留给云同步架构，与 13.6 同 tip、无独有提交。锁加固不进本线 | 停放；不要在这条上改锁 |
| `v1` | `0c2c753` | 2026-09-10 | EN: Stage1 tip follows v1.13.7 Wave 1. / 中文：大阶段 1 tip 跟随 13.7 Wave 1 | `main` / `release/1.0.0` 不动 |
