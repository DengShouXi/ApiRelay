...
> 更新时间：2026-09-09。下列 `tip` 记的是**该迭代做完代码的那一次提交**；紧随其后「写本文件」的 docs 提交不计入，因此分支真实尖端通常比本列多一格（例：`v1.13` 本列 `37d7bd9`，分支与 tag `release/1.0.0` 实际都在 `539447e`）。这不是台账落后，是记录本记不下它自己造成的那一格。勿把 tip 回退，以免破坏已冻结里程碑。  
> **当前：T062 已执行** — `main` + tag `release/1.0.0`（Stage1 → 商店 **1.0.0** 仍在架上）。  
> **工作分支**：`v1.13.4` — 商店号仍 **1.0.1**；已传 TestFlight 的是构建 **5**；源码构建号已钉 **6**（下一包 Archive 用）。本线已入库时长子页 / 权益放行 / 规格防回退。冻结：`v1.13` / `v1.13.1` / `v1.13.2` / `v1.13.3` 与 tag `release/1.0.0` 不动。  
> **下一步**：现有 TestFlight **1.0.1 (5)** 仍可手测（不含本次时长 JSON）。发出含时长字段的下一包前，须 CloudKit Production additive Deploy，再 Archive **1.0.1 (6)**。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」。  
