...
| `v1.13.8` | `7ca8727` | 2026-09-10 | EN: Hotfix 13.8 opened — identity-auth scheme (three settings rows; factory default device-owner; reveal reuse same detail page; admin credentials stay mandatory device-owner). Specs in `身份验证方案.md` and `001-key-vault/follow-ups/identity-auth/`. No Swift yet. Store still **1.0.1**, build still **9**. / 中文：热修 13.8 开线 — 身份验证方案（三行；出厂设备验证；取用同页复用；管理类凭证仍强制设备主人）。规格在 `身份验证方案.md` 与 `follow-ups/identity-auth/`。尚未改 Swift。商店号仍 1.0.1，构建号仍 9 | 工作分支；不覆盖 `v1.13.7`；`main` / tag 不动 |
