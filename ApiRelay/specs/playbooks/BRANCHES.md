...
> **工作分支**：仍是 `v1.13.2` 上架后热修（2026-09-04 续：**H3c 救活预览已执行**——`makePreview` + Vault/Settings Canvas）；冻结的 `v1.13`、`v1.13.1` 与 tag `release/1.0.0` 不动。  
> **下一步**：备案、商店热修 1.0.1；**H4 / 99**；旁账（目录改名 / `tools/`）。全部确认后再另行授权商店 `1.0.1`、`main` 与 tag `release/1.0.1`。  
