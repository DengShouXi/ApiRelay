...
> **工作分支**：`v1.13.6` — App 锁生命周期（遮罩 / 上锁 / 系统验证拆开；会话锁是业务闸）。商店号仍 **1.0.1**；源码构建号仍 **7**。冻结：`v1.13` … `v1.13.5` 与 tag `release/1.0.0` 不动。  
> **下一步**：锁验收矩阵手测（`锁-验收矩阵.md`）；CloudKit 卫生双设备 T008 仍欠。云同步进一步调整**不进本线**。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」、改构建号 9。  
