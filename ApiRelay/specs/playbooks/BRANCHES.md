...
> **工作分支**：`v1.13.5` — CloudKit 同步卫生（读去重 / 写打全 / 启动与导入清扫）。商店号仍 **1.0.1**；已传 TestFlight 的是构建 **5**（无时长 JSON、无卫生）和 **6**（2026-09-09，时长/权益，无卫生）。源码构建号 **7**（6 已被占用）。冻结：`v1.13` / `v1.13.1` / `v1.13.2` / `v1.13.3` / `v1.13.4` 与 tag `release/1.0.0` 不动。  
> **下一步**：时长 JSON 两字段 **2026-09-10 已在 Production**（Deploy 差异为 0）。从本线 Archive **1.0.1 (7)**（含卫生），再双设备手测（T008）。不要分发 Organizer 里重复的 1.0.1 (6)。`replicaSeed` 本热修不做。`tools/` 入库与备案仍不急。另授权才 ff `main`、打 tag `release/1.0.1`、点 ASC「提交审核」。  
