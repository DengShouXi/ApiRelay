...
编辑 `BRANCHES.md`，为小迭代 **`v3.3`** 写入/更新：

1. **English**（2–4 句）：相对上一小迭代多了什么、不含什么、何时 `git checkout v3.3`。  
2. **简体中文**：同样信息。  
...
6. 建议 message：`feat(v3.3): Phase 3 托管与签发 iteration`  
7. 上传本小迭代：

```bash
git push -u origin v3.3
```
...
- 下一步：打开下一 Phase **实现**提示词；保存时再开下一小迭代 `v3.4`  

## 禁止

- 用 Git tag 充当小迭代名（禁止；上架才打 `release/N.0.0`；迭代一律用分支）  
- 预建未开始的大阶段 / 小迭代（如 `v3.4` 要等下一 Phase 完成再开）  
