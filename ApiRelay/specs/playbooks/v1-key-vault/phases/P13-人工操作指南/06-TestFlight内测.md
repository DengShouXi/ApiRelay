...
| 3 | 复制密钥 | **定稿出厂默认是设备验证**（2026-09-10 在 13.8 拍板，落地计划尚未创建的 13.9）。含新验证的包应先过系统设备主人再复制；若用户改成「不验证」则直接复制。现行未改代码的包仍可能是旧默认「不验证」 |
| 4 | 按使用方指派 | 双视角能看到指派 |
| 5 | 设置 → **恢复购买** | 入口在；点一下不崩（沙盒无购买也 OK） |
| 6 | 设置 → 验证方式 | 查看与复制仍共用一道门闩。含新验证的包另有「取用验证」行（落地计划 13.9）。四档名称见 `身份验证方案.md`，不要再按「仅生物识别」测新包 |
