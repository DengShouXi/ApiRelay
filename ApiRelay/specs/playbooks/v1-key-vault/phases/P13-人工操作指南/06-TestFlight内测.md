...
| 3 | 复制密钥 | **规格出厂默认是设备验证**（宪法 v2.15.0，列表名「设备验证」）。含新验证的包且取用验证开启时应先过当前验证方式再复制；若用户改成「不验证」或关掉取用验证则直接复制。商店/TestFlight 上尚未包含本次本机改动的旧包，仍可能是旧默认「不验证」 |
| 4 | 按使用方指派 | 双视角能看到指派 |
| 5 | 设置 → **恢复购买** | 入口在；点一下不崩（沙盒无购买也 OK） |
| 6 | 设置 → 验证方式 | 查看与复制共用**取用验证**，不得拆成两个开关。规格另有独立「取用验证」行（出厂开启）。四档名称见 `身份验证方案.md`：设备验证档固定叫「设备验证」。点应用密码或组合档都进同一应用密码页（已设也进）；未设时立即设密。设置首页仅在已 persist 为这两种档时显示「应用密码」行。当前 `v1.13.9` 工作区用 Xcode Run 的本机新包才是这套行为；商店/旧 TestFlight 包还不是 |
