...
本对话只做：写备注 + commit + 推送到小迭代分支 `plan.2`。不要夹带产品实现代码（除非该迭代本身需要）。
...
|----|----------------|
| 大阶段分支 | `plan` |
| **本小迭代分支（本次上传目标）** | `plan.2` |
| 下一小迭代（本步不要创建，除非我另说） | `plan.3` |
...
git checkout plan
git pull
git checkout -B plan.2
```

3. 暂存本迭代文档/规划改动 + `BRANCHES.md`（及 `V0/Phase02_SpecsFreeze/` 若有测试）。  
4. 禁止：`build/`、`DebugScratch/`、密钥、`spec-kit-0.15.2/`。  
5. 展示 cached diff，**同意后再 commit**：`docs(plan.2): Phase 2 规格冻住 iteration`  
6. `git push -u origin plan.2`  
7. 推荐同步大阶段：`git checkout plan && git merge plan.2 && git push origin plan && git checkout plan.2`

## C. 汇报

- `plan.2` @ `<hash>` 已在远程  
- 备注英+中已写  
- 下一步：若还要规划 → `P03` / `plan.3`；若开始产品实现 → 到 `v1` / `v1.N` 线  

## 禁止

- 用 tag 代替 `plan.2`（禁止；规划迭代一律用分支）  
- 未做完就预建更后面的迭代  
- force push  
