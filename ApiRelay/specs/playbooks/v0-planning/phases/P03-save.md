...
本对话只做：写备注 + commit + 推送到小迭代分支 `v0.0.3`。不要夹带产品实现代码（除非该迭代本身需要）。
...
|----|----------------|
| 大阶段分支 | `v0.0` |
| **本小迭代分支（本次上传目标）** | `v0.0.3` |
| 下一小迭代（本步不要创建，除非我另说） | `v0.0.4` |
...
5. 展示 cached diff，**同意后再 commit**：`docs(v0.0.3): Phase 3 规划补充 iteration`  
6. `git push -u origin v0.0.3`  
7. 推荐同步大阶段：`git checkout v0.0 && git merge v0.0.3 && git push origin v0.0 && git checkout v0.0.3`

## C. 汇报

- `v0.0.3` @ `<hash>` 已在远程  
- 备注英+中已写  
- 下一步：若还要规划 → `P04` / `v0.0.4`；若开始产品实现 → 到 `v0.1` / `v0.1.N` 线  
