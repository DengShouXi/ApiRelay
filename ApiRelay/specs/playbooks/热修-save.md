...
> **上传顺序权威**：[`ZL01_具体说明/09-上传流程.md`](../../../ZL01_具体说明/09-上传流程.md) §1。本文件只管 git（开分支、台账、commit、push）。  
> **分支权威**：[`BRANCHES.md`](./BRANCHES.md)「分支 vs tag」+ 保护约定。中文说明：[`重要说明/分支与版本.md`](./重要说明/分支与版本.md)。

用户说「上传」即可，不必把本文件贴进对话，也不必再念「按热修-save」。  
进入下面 A 节之前，MUST 已按 `09` §1 做完对照规格与同步 `ZL01`。若本次为对齐改了代码，`selfcheck` 红线不过则不要进入 commit。

对齐规格所必需的改动允许在「上传」对话里做完（见 `09` §1.1）；禁止顺手重构。不要送审、不要打新 tag。

---

请按 `ZL01_具体说明/09-上传流程.md` §1 做完对照与同步后，再按本文件保存并上传**当前工作区已确认可入库的改动**（上架后热修，不是新的 P0N）。
