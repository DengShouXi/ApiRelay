<!--
Sync Impact Report（最新：v2.16.0 → v2.17.0）
==========================================
Version change: 2.16.0 → 2.17.0 (MINOR: 组合档系统认证改为单上下文生物识别→设备密码，应用密码保持显式独立入口)

Modified principles / requirements:
  - VIII. 身份确认门闩
      * 组合档的系统路径只启动一次 `deviceOwnerAuthentication`，由 Apple 在同一系统流程中先生物识别、再直接回落设备密码或 Mac 登录密码。
      * MUST NOT 在生物失败后新建第二个系统认证上下文；否则会重新触发面容/指纹。
      * 应用密码继续使用 App 内显式入口；系统取消不得自动改走应用密码。

Templates / downstream sync:
  - `specs/001-key-vault/spec.md`、`research.md`、`quickstart.md`、`contracts/module-interfaces.md`
  - `follow-ups/identity-auth/spec.md`、`playbooks/身份验证方案.md`

Follow-up TODOs: 当前新构建在原 iPhone / Mac 真实验证系统回落；三端其余手测仍待做

历史记录
========
Sync Impact Report（v2.15.0 → v2.16.0）
==========================================
Version change: 2.15.0 → 2.16.0 (MINOR: 共用应用密码页、界面统一称应用密码、按已保存档显示管理行)

Modified principles / requirements:
  - VIII. 身份确认门闩
      * 界面统一称「应用密码」，不把设置行写成应用主密码。
      * 点应用密码或组合档都进同一页，已设也不跳过；进页不改当前档。
      * 设置首页仅在已保存为这两种档时显示应用密码行；其余档隐藏行但保留材料。
      * 选档页与锁屏缺材料恢复不依赖首页行。U1 的立即设密、普通入口不设密、列表定名未删。

Templates / downstream sync:
  - `specs/001-key-vault/spec.md`（FR-003 / 021 / 037）
  - `quickstart.md`、`plan.md`、`research.md`、`tasks.md`、`checklists/requirements.md`
  - `follow-ups/identity-auth/spec.md`、`playbooks/身份验证方案.md`、P13、ZL01/04、ZL01/14

Follow-up TODOs: v1.13.9 按 U2 后续波次改 Swift；三端手测仍待做

历史记录
========
Version change: 2.14.0 → 2.15.0 (MINOR: 设备验证列表定名、密码档立即设密、独立应用主密码行、普通入口不设密)

Modified principles / requirements:
  - VIII. 身份确认门闩
      * 设备验证档列表标题固定为「生物验证或设备密码」，不随硬件改列表名。
      * 选中应用密码或组合档且本机未设密码时，立即走准备流程；复查成功后才 persist 目标档。
      * 设置首页始终可见「应用主密码」行；首次设置须设备主人，修改须旧密码。
      * 普通解锁/取用/编辑/删除/备份入口不得设密；缺材料走独立恢复，恢复丢弃原待办。
      * 应用密码是本机门闩，不是整库再加密；设备主人不能阻止知道设备密码的人做显式恢复。
      * 四档、出厂设备验证、取用复用、组合档取消不回落设备密码、13.7 锁生命周期未删。

Templates / downstream sync:
  - `specs/001-key-vault/spec.md`（FR-003 / 003a / 021 / 036 / 037 / 066）
  - `specs/001-key-vault/data-model.md`、`quickstart.md`、`plan.md`、`research.md`、`tasks.md`
  - `specs/001-key-vault/contracts/module-interfaces.md`
  - `specs/001-key-vault/follow-ups/identity-auth/spec.md`
  - `specs/playbooks/身份验证方案.md` 及 P03/P06/P07/P13、锁资料

Follow-up TODOs: v1.13.9 按 U1 后续波次改 Swift；三端手测与双设备仍待做

历史记录
========
Version change: 2.13.0 → 2.14.0 (MINOR: 四档身份验证、三行设置、取用复用与当前方式降级)

Modified principles / requirements:
  - VIII. 身份确认门闩
      * 出厂默认设备验证；四档不再含「仅生物识别」。
      * 设置三行：验证方式 / 自动锁定 / 取用验证（取用出厂开启）。
      * 查看后同一详情内立刻复制可复用授权；导出/删除不再永远强制设备主人。
      * 降低安全与三种有验证方式互换须用当前验证方式确认；管理类凭证仍强制设备主人。
      * 「生物验证或应用密码」取消不得改弹设备密码。13.7 锁生命周期不变量未删。

Templates / downstream sync:
  - `specs/001-key-vault/spec.md`（FR-003 / 003a / 006 / 006a / 021 / 036 / 060 / 061 / 066）
  - `specs/001-key-vault/data-model.md`、`quickstart.md`、`plan.md`、`research.md`、`tasks.md`
  - `specs/001-key-vault/contracts/module-interfaces.md`
  - `specs/001-key-vault/follow-ups/identity-auth/spec.md`
  - `specs/playbooks/身份验证方案.md` 及 P03/P06/P07/P13、锁资料

Follow-up TODOs: v1.13.9 W1 起改 Swift；锁矩阵 Wave 4 三端手测仍建议等身份验证代码
...
门闩挡的是「**用户取出凭证明文**」以及需要身份确认的敏感操作，而非系统内部的读取行为。

用户在设置中选择**三行**：验证方式、自动锁定、取用验证。验证方式决定**怎么验**；后两行决定这两种场合**用不用**。编辑、回收站、备份、永久删除、清空不另开开关。

验证方式 MUST 按顺序提供四档：**设备验证**（出厂默认、推荐）、**应用验证**、**设备验证或应用验证**、**不验证**。列表名 MUST 固定，不随硬件改成面容 ID / 触控 ID；ⓘ 与系统弹窗使用 Apple 官方译名。设备验证使用 `deviceOwnerAuthentication`，优先生物识别，系统回落设备密码（Mac 登录密码）。组合档的系统路径同样只启动一次 `deviceOwnerAuthentication`：由 Apple 在同一系统流程内先使用本机生物识别，失败、锁定、不可用或用户点「使用设备密码」后直接进入设备密码 / Mac 登录密码；MUST NOT 先跑 `deviceOwnerAuthenticationWithBiometrics`、返回 App 后再新建第二个系统认证上下文，因为那会重新触发生物识别。应用密码保持 App 内显式入口，仅校验已配置材料；系统取消不得自动改走应用密码。取消整段流程、离开页面或进入后台 MUST 结束，不得自动执行业务或创建密码。旧 rawValue 保留兼容。

选中「应用密码」或「设备验证或应用验证」时，MUST 进入**同一个**应用密码页面，并绑定用户所选目标档；本机已经设过密码也 MUST 进入，MUST NOT 因已设而跳过页面直接保存。进入该页 MUST NOT 改变当前验证方式。若目标已是当前档，该页只做管理，MUST NOT 制造一次切换。查询材料 MUST 区分未设 / 已设 / 读取错误；读取错误 MUST NOT 冒充未设、MUST NOT 允许覆盖。未设则显示创建：先按当前验证方式确认（从不验证加强可免），再单独完成设备主人验证，用户输入新密码并确认，成功写入 Keychain 并复查后才 persist 目标档。已设则显示已设置、修改、显式恢复，以及「保留现有密码并继续使用此验证方式」；MUST NOT 强迫改密。继续使用时按原规则完成当前方式验证后 persist 原目标。修改密码成功 MUST NOT 被当成验证方式已保存。设密目标 MUST 保持用户所选档，MUST NOT 把组合档硬编码成应用密码档。MUST NOT 用纯生物成功代替新设密所需的设备主人验证。任一步失败或取消：当前档与会话 MUST NOT 提前变化，MUST NOT 有业务副作用或成功提示。Keychain 写入成功但策略 persist 失败时，MUST 保留新材料与旧档，MUST 提示密码已设置但验证方式未切换，并允许重试；MUST NOT 盲目删除已写材料。MUST NOT 等到解锁、取用、删除或备份时才设。

设置首页的「应用密码」行 MUST 仅在当前已成功保存为「应用密码」或「设备验证或应用验证」时显示（本机已设置 / 未设置 / 读取失败可重试）。待选目标、表单打开或本机已有材料均 MUST NOT 提前让该行出现。切到「设备验证」或「不验证」MUST 隐藏该行，MUST NOT 删除本机密码材料；再选回这两种档且材料仍在时 MUST 进入已设管理，MUST NOT 再当首次设密。用户界面（标题、状态、提示、辅助标签）MUST 称「应用密码」，MUST NOT 再写成「主密码」或「应用主密码」。MUST NOT 显示密码内容，MUST NOT 与备份口令混淆。首次设置 MUST 经设备主人；日常修改 MUST 校验旧应用密码；忘记或缺材料恢复 MUST 经用户显式确认并设备主人验证，成功后落到设备验证，MUST NOT 落到「不验证」，MUST NOT 删除密钥或备份口令。删除材料前 MUST 先安全 persist 回退策略，失败 MUST NOT 删除。缺材料时选档页与锁屏的独立恢复 MUST 始终可达，MUST NOT 依赖首页该行是否显示。

普通解锁、取用、编辑、删除、备份、降低安全入口 MUST NOT 设置应用密码。组合档缺材料时系统设备验证（生物识别或设备密码）仍可用，应用密码路径 MUST 拒绝并指向独立管理或恢复。应用密码档缺材料 MUST NOT 要求一个不存在的旧密码才能恢复。恢复 MUST 丢弃原待办，MUST NOT 自动解锁或执行原业务。已有密码输错 MUST NOT 借首次设置覆盖。

设备主人验证意味着知道设备密码的人也能完成显式恢复。应用密码是本机门闩，MUST NOT 被描述为重新加密整个密钥库。

取用验证出厂默认**开启**。开启且验证方式不是「不验证」时，查看明文与复制到剪贴板 MUST 按当前验证方式确认。二者 MUST 共用这一开关，MUST NOT 拆成「只验查看不验复制」。同一密钥详情实例内，查看成功后立刻复制 MUST 复用该次授权，不得再弹；关详情、换密钥、离开前台、自动锁定或会话锁后 MUST 立即失效。该授权 MUST NOT 用于导出、删除、清空、编辑或另一窗口。

「不验证」时：冷启动与再进 MUST 直接进列表，MUST NOT 画假锁屏；查看、复制、编辑、移入回收站、恢复、备份导入导出直接走（移入回收站 MAY 保留普通确认）。永久删除与清空全部数据 MUST 仍经不可关闭的破坏性确认。自动锁定与取用验证的开关及时长 MUST 保留，界面显示「已开启，当前不生效」，MUST NOT 置灰。

验证方式不是「不验证」时：编辑、移入回收站、恢复、备份导入导出 MUST 按**当前验证方式**确认。永久删除与清空 MUST 先破坏性确认，再按当前验证方式确认。新增密钥 MUST 不另验身份。

查看或配置管理类高权限凭证的门闩 MUST NOT 可被用户关闭，MUST 始终使用设备主人认证（`confirmMandatory`），不受「不验证」影响。
...
- 降低已同步的安全等级（关闭自动锁定、拉长自动锁、关闭取用验证、改为「不验证」、关闭多任务隐藏、
  关闭剪贴板自动清除）以及三种需要身份验证的方式互相切换，MUST 先按**当前验证方式**确认。
  加强安全（从不验证改到其他档、开启自动锁定或取用验证、缩短自动锁）MUST 立即生效，可直接保存。
  降低等级 MUST 在同步偏好 persist **成功后**才改内存锁态、启动缓存与界面；失败 MUST 保持原状
  （含会话仍锁、缓存仍为开）。  加强仍可先改内存。忘记或缺材料恢复应用密码 MUST 先经用户显式确认并过设备主人，成功后
  只重置本机门闩材料并落到设备验证，MUST NOT 改成「不验证」，MUST NOT 删除密钥明文。
...
**Rationale**: 即使设备已解锁，取出凭证仍宜二次身份确认，防范未授权物理访问；同时允许用户主动选择
「不验证」，并如实告知风险。查看与复制产出同一份明文，故取用验证必须同时管辖二者，不得分设开关。
同一详情内看完立刻复制再弹一次没有额外安全收益，只增加摩擦；该复用不得扩大到导出或删除。
管理类凭证可创建密钥并产生消费，门闩不可关且始终设备主人。加密导出在「不验证」时可直达，是用户
已接受「他人拿到已解锁设备即可导出」这一风险后的产品决定，不是实现疏漏。
...
**Version**: 2.17.0 | **Ratified**: 2026-08-04 | **Last Amended**: 2026-09-22

<!--
v2.17.0（MINOR，组合档系统认证单上下文回落）：
  - 系统路径统一为一次 `deviceOwnerAuthentication`，先生物识别、同一系统流程直接回落设备密码 / Mac 登录密码。
  - 禁止生物失败后新建第二个系统认证上下文；应用密码保持 App 内显式独立入口。
  - Sync Impact：FR-003a、identity-auth、身份验证方案、research、quickstart、module-interfaces。

v2.16.0（MINOR，共用应用密码页与按档显示管理行）：
  - VIII：界面统一称「应用密码」；点应用密码或组合档都进同一页，已设也不跳过；
    设置首页仅在已保存为这两种档时显示应用密码行；选档页与锁屏恢复不依赖该行。
    U1 的立即设密、普通入口不设密、列表定名未删。
  - Sync Impact：spec FR-003/021/037、quickstart、plan、research、tasks、requirements、
    follow-ups/identity-auth、playbooks/身份验证方案.md、P13、ZL01/04、ZL01/14。

v2.15.0（MINOR，设密前置与设备验证定名）：
  - VIII：列表标题固定「生物验证或设备密码」；密码依赖档未设则立即准备；始终可见应用主密码行；
    普通入口不设密；缺材料独立恢复；Keychain 与偏好分步失败须如实提示。
  - Sync Impact：spec FR-003/003a/021/036/037/066、data-model、contracts、quickstart、
    research、playbooks/身份验证方案.md、P03/P06/P07/P13。

v2.14.0（MINOR，四档身份验证与三行设置）：
  - VIII：出厂设备验证；取用验证独立且默认开；同详情取用复用；降低与三档互换用当前方式；
    导出/删除不再永远强制设备主人；组合档取消不回落设备密码；管理类凭证仍强制设备主人。
    13.7 锁生命周期条款未删。
  - Sync Impact：spec FR-003/006/021/036/060/061/066、data-model、contracts、quickstart、
    research、playbooks/身份验证方案.md、P03/P06/P07/P13、锁资料。
