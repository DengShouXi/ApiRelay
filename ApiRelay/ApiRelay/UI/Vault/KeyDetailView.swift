...
struct KeyDetailView: View {
    ...
    @State private var awaitingViewUnlock = false
    @State private var reuseInstanceID = UUID()
    ...
    var body: some View {
        ...
        .onChange(of: revealedSecret) { _, result in
            guard let result else { return }
            if awaitingViewUnlock {
                awaitingViewUnlock = false
                revealedSecret = nil
                displayViewedSecret(result)
                return
            }
            guard awaitingEditUnlock else { return }
            awaitingEditUnlock = false
            revealedSecret = nil
            beginEditing(with: result.secret)
        }
        .onReceive(viewModel.combinationDeliveredSecretPublisher) { payload in
            guard payload.keyId == keyId else { return }
        ...
        .onChange(of: scenePhase) { _, phase in
            if phase != .active {
                ...
                // A real app switch/background transition is a security boundary:
                // the edit form contains the complete secret, not just metadata.
                // Keep the form only for the transient `.inactive` generated by
                // this app's own Face ID / Touch ID / device-password sheet.
                let isTransientSystemAuthentication = phase == .inactive
                    && viewModel.environment.gate.isAuthenticationInProgress()
        ...
        .onDisappear {
            viewModel.invalidateRevealReuseWindowDestroyed()
            leaveEditing(clearUnlock: true)
            applyInvalidatedRevealDisplay()
        }
        .toolbar {
    ...
    /// 查看成功后明文只留在本详情实例；复制走已过门闩的 `copyRevealedSecret`。
    @ViewBuilder
    private func secretBrowseRow(_ key: KeyRecordDTO) -> some View {
        if key.secretAvailable {
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .firstTextBaseline) {
                    Text("vault.detail.secret")
                        .foregroundStyle(.secondary)
                    Spacer(minLength: 12)
                    if isSecretShown, let displayedSecret, !displayedSecret.isEmpty {
                ...
                HStack(spacing: 16) {
                    Button(isSecretShown ? "vault.detail.hideSecret" : "vault.detail.showSecret") {
                        if isSecretShown {
                            isSecretShown = false
                        } else if let displayedSecret, !displayedSecret.isEmpty {
                            isSecretShown = true
                        } else {
                            Task { await requestViewing() }
                        }
                    }
                    .accessibilityLabel(
                        Text(isSecretShown ? "vault.a11y.hideSecret" : "vault.a11y.showSecret")
                    )
                    Button("vault.copy") {
                        Task { await copyFromDetail(key) }
                    }
                    .accessibilityLabel(Text("vault.a11y.copySecret"))
                    if viewModel.offerCombinationAppPassword {
                        Button("appLock.useAppPassword") {
                            Task {
                                awaitingViewUnlock = isSecretShown == false
                                await viewModel.beginCombinationAppPasswordEntry()
                            }
                        }
            ...
            .accessibilityLabel(
                Text(isSecretShown ? "vault.a11y.secretVisible" : "vault.a11y.secretHidden")
            )
        } else {
    ...
    private func requestViewing() async {
        guard let key else { return }
        awaitingViewUnlock = false
        if let result = await viewModel.revealReturning(keyId: key.id, masterPassword: nil) {
            displayViewedSecret(result)
            return
        }
        if viewModel.needsMasterPassword {
            awaitingViewUnlock = true
            onRequestMasterPassword(key.id)
            return
        }
        if viewModel.offerCombinationAppPassword || viewModel.hasPendingSensitiveRetry {
            awaitingViewUnlock = true
        }
    }

    private func copyFromDetail(_ key: KeyRecordDTO) async {
        if let token = viewModel.takeRevealReuseTokenIfAllowed(
            instanceID: reuseInstanceID,
            keyId: key.id
        ) {
            await viewModel.copyRevealedSecret(keyId: key.id, token: token)
            return
        }
        onCopy(key.id)
    }

    private func applyInvalidatedRevealDisplay() {
        displayedSecret = nil
        isSecretShown = false
        awaitingViewUnlock = false
    }

    private func displayViewedSecret(_ result: SecretRevealResult) {
        displayedSecret = result.secret
        isSecretShown = !result.secret.isEmpty
        viewModel.noteRevealDisplay(
            instanceID: reuseInstanceID,
            keyId: keyId,
            result: result
        )
    ...
    private var editSecretRow: some View {
        HStack(alignment: .firstTextBaseline) {
            ...
            TextField("", text: $draftSecret)
                ...
                .accessibilityLabel(Text("vault.detail.secret"))
                #if os(iOS) || targetEnvironment(macCatalyst)
    ...
    private var editNotesRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField("", text: $draftNotes, axis: .vertical)
                .lineLimit(3...8)
                .textFieldStyle(.plain)
                .accessibilityLabel(Text("vault.detail.notes"))
        }
        .padding(.vertical, 12)
    }
    ...
    private func commitAvatarIfBrowsing() async {
        guard !isEditing, let key else { return }
        await viewModel.updateKeyAvatar(
            keyId: key.id,
            usesDefault: usesDefaultAvatar,
            avatar: customAvatar
        )
    ...
    private func requestEditing() async {
        ...
        awaitingEditUnlock = false
        awaitingViewUnlock = false
        viewModel.invalidateRevealReuseBeginEdit()
        applyInvalidatedRevealDisplay()
