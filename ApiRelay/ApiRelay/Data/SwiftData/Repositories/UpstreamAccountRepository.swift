...
nonisolated struct UpstreamAccountStorageSnapshot: Sendable {
    let id: UUID
    let platform: String
...
actor UpstreamAccountRepository {
    ...
    func storageSnapshot(id: UUID) throws -> UpstreamAccountStorageSnapshot? {
        guard let model = SyncedIdentity.winner(in: try fetchModels(id: id), rank: Self.rank) else {
            return nil
        }
        return UpstreamAccountStorageSnapshot(
            id: model.id,
    ...
    func restoreStorageSnapshot(_ snapshot: UpstreamAccountStorageSnapshot) throws {
        let models = try fetchModels(id: snapshot.id)
        if models.isEmpty {
            modelContext.insert(UpstreamAccount(
                id: snapshot.id,
                platform: snapshot.platform,
                customPlatformName: snapshot.customPlatformName,
                displayName: snapshot.displayName,
                customBaseURL: snapshot.customBaseURL,
                hasManagementCredential: snapshot.hasManagementCredential,
            ...
            for model in models {
                model.platform = snapshot.platform
                model.customPlatformName = snapshot.customPlatformName
                model.displayName = snapshot.displayName
                model.customBaseURL = snapshot.customBaseURL
                model.hasManagementCredential = snapshot.hasManagementCredential
